        ...
        void draw(std::vector<cv::Mat>& frames) {
            ...
            for(int bi=0; bi<frames.size(); ++bi){
                ...
                for(int i=0; i<batchDetected[bi].size(); i++) { 
                    ...
                    oss << det_class << " " << std::fixed << std::setprecision(2) << prob * 100 << "%"; 
                    cv::putText(frames[bi], oss.str(), cv::Point(x0, (y0 - (baseline / 2))), cv::FONT_HERSHEY_SIMPLEX, font_scale, cv::Scalar(255, 255, 255), thickness);
                }
            }
        }
